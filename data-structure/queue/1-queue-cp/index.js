...
class Queue {
    #lines //antrian private
    constructor(){
        this.#lines = []; //membuat antrian kosong
    }

    //melakukan push ke antrian
    enqueue(queue){
        this.#lines.push(queue);
    }

    //menghapus antrian pada head
    dequeue(){
        this.#lines.shift();
    }
    ...
    //getter untuk return semua antrian
    get lines(){
        return this.#lines;
    }

    //getter untuk return head (antrian terdepan)
    get head(){
        return this.#lines[0];
    }
}
...
function countStudentsCantEat(students, sandwiches) {
    ...
    return studentsQueue.lines.length;
}
